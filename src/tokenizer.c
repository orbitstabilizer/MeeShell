...
void tokenizer_next(Tokenizer *self){
    ...
    if (c == '>'){
        ...
        if (c == '>'){
            ...
            if (c == '>'){
                self->cur_pos++;
                init_token(token, TOKEN_ANGLE_BRACKET3, 3, &self->input[self->cur_pos - 3]);
            }else{
                init_token(token, TOKEN_ANGLE_BRACKET2, 2, &self->input[self->cur_pos - 2]);
            }
        }else{
            init_token(token, TOKEN_ANGLE_BRACKET1, 1, &self->input[self->cur_pos - 1]);
        }
        self->cur_token++;
        return;
    }
    ...
    if (self->quote){
        ...
        init_token(token, TOKEN_LITERAL, len, &self->input[self->cur_pos - len]);
        self->cur_token++;
        return;
    }
    else{
